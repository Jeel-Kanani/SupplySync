...
const navGroups = [
  ...
  },
  {
    label: 'Automation',
    items: [
      { label: 'Overview', path: '/automation', icon: FiActivity },
...
export const Sidebar = ({ open, onClose }) => {
  ...
  return (
    ...
    />
    <aside
      className={`fixed inset-y-0 left-0 z-40 flex w-72 transform flex-col border-r border-gray-800 bg-gray-950 px-4 py-5 text-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${
    ...
    >
      ...
      <nav className="mt-8 flex-1 space-y-6 overflow-y-auto pr-1">
        {navGroups.map((group) => (
          <div key={group.label}>
            <p className="px-3 text-xs font-semibold uppercase tracking-normal text-gray-500">{group.label}</p>
